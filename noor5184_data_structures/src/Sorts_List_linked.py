...
class Sorts:
    ...
    def radix_sort(a):
        ...
        def _radix_nonneg(lst):
            if len(lst) > 0:
                ...
                while max_value // exp > 0:
                    ...
                    while lst._front is not None:
                        value = lst._front._value
                        digit = (value // exp) % 10
                        buckets[digit]._move_front_to_rear(lst)

                    for bucket in buckets:
                        while bucket._front is not None:
                            lst._move_front_to_rear(bucket)
        ...
        if not a.is_empty():
            ...
            while a._front is not None:
                value = a.remove_front()
                if value < 0:
                    negatives.append(-value)
                else:
                    nonnegatives.append(value)

            _radix_nonneg(negatives)
            _radix_nonneg(nonnegatives)

            while negatives._front is not None:
                value = negatives.remove_front()
                a.prepend(-value)

            while nonnegatives._front is not None:
                a._move_front_to_rear(nonnegatives)
        return
