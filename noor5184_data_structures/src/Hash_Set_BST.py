...
class Hash_Set:
    ...
    def insert(self, value):
        """
        -------------------------------------------------------
        Inserts a Word object into the appropriate BST bucket.
        If the word is already present in that BST, update its
        .comparisons counter instead of adding a duplicate.
        -------------------------------------------------------
        Parameters:
            value - a Word object (Word)
        Returns:
            None
        -------------------------------------------------------
        """
        # Which bucket (slot) does this Word belong to?
        slot_index = self._hash(value)          # hash(key) % capacity
        slot = self._table[slot_index]          # slot is a BST
    
        # Does this BST already contain the Word?
        found = slot.retrieve(value)            # returns the stored Word or None
    
        if found is not None:
            # Word already exists in this slot – just accumulate comparisons
            found.comparisons += value.comparisons
        else:
            # New word – insert into BST and update overall count
            slot.insert(value)
            self._count += 1
