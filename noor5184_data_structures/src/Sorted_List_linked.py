...
class Sorted_List:
    ...
    def insert(self, value):
        ...
        # Find insertion point (after existing equals for stability)
        prev = None
        cur = self._front
    ...
    def _linear_search(self, key):
        ...
        # stop once current value is >= key (first possible match)
    ...
    def remove(self, key):
        ...
        prev, cur, _ = self._linear_search(key)
        value = None
        ...
        if cur is not None:
            ...
            if prev is None and self._front is None:
                self._rear = None
            if prev is not None and prev._next is None:
                self._rear = prev
            self._count -= 1
    ...
    def find(self, key):
        ...
        value = deepcopy(cur._value) if cur is not None else None
    ...
    def index(self, key):
        ...
        _, cur, i = self._linear_search(key)
        index = i if cur is not None else -1
    ...
    def count(self, key):
        ...
        _, cur, _ = self._linear_search(key)
        n = 0
        while cur is not None and cur._value == key:
            n += 1
            cur = cur._next
    ...
    def intersection(self, source1, source2):
        ...
        while p is not None and q is not None:
            if p._value < q._value:
                p = p._next
            elif q._value < p._value:
                q = q._next
            else:
                # equal
                if last_val is None or p._value != last_val:
                    node = _SL_Node(p._value, None)
                    if self._rear is None:
                        self._front = node
                        self._rear = node
                    else:
                        ...
                        self._rear = node
                    self._count += 1
                    last_val = node._value
                # advance both (skip duplicates in either)
                val = p._value
                while p is not None and p._value == val:
                    p = p._next
                while q is not None and q._value == val:
                    q = q._next
    ...
    def union(self, source1, source2):
        ...
        while p is not None and q is not None:
            if p._value < q._value:
                append_value(p._value)
                pv = p._value
                while p is not None and p._value == pv:
                    p = p._next
            elif q._value < p._value:
                append_value(q._value)
                qv = q._value
                while q is not None and q._value == qv:
                    q = q._next
            else:
                append_value(p._value)
                v = p._value
                while p is not None and p._value == v:
                    p = p._next
                while q is not None and q._value == v:
                    q = q._next
