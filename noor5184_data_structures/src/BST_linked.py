...
class BST:
    ...
    def __eq__(self, target):
        ...
        def _eq_aux(node1, node2):
            if node1 is None and node2 is None:
                ...
                left_eq = _eq_aux(node1._left, node2._left)
                right_eq = _eq_aux(node1._right, node2._right)
                result = left_eq and right_eq
            return result

        if not isinstance(target, BST):
            equals = False
        else:
            equals = _eq_aux(self._root, target._root)
        return equals
    ...
    def parent_r(self, key):
        ...
        def _parent_r_aux(node, parent, key):
            if node is None:
                result = None
            elif key < node._value:
                result = _parent_r_aux(node._left, node, key)
            elif key > node._value:
                result = _parent_r_aux(node._right, node, key)
            else:
                if parent is not None:
                    result = deepcopy(parent._value)
                else:
                    result = None
            return result

        value = _parent_r_aux(self._root, None, key)
        return value
    ...
    def max_r(self):
        ...
        def _max_r_aux(node):
            if node._right is None:
                result = deepcopy(node._value)
            else:
                result = _max_r_aux(node._right)
            return result

        value = _max_r_aux(self._root)
        return value
    ...
    def min_r(self):
        ...
        def _min_r_aux(node):
            if node._left is None:
                ...
                result = _min_r_aux(node._left)
            return result

        value = _min_r_aux(self._root)
        return value
    ...
    def leaf_count(self):
        ...
        def _leaf_count_aux(node):
            if node is None:
                count = 0
            elif node._left is None and node._right is None:
                count = 1
            else:
                count = _leaf_count_aux(node._left) + _leaf_count_aux(node._right)
            return count

        count = _leaf_count_aux(self._root)
        return count
    ...
    def node_counts(self):
        ...
        def _node_counts_aux(node):
            if node is None:
                zero = 0
                one = 0
                two = 0
            else:
                l_zero, l_one, l_two = _node_counts_aux(node._left)
                r_zero, r_one, r_two = _node_counts_aux(node._right)
                zero = l_zero + r_zero
                one = l_one + r_one
                two = l_two + r_two
        ...
        zero, one, two = _node_counts_aux(self._root)
        return zero, one, two
    ...
    def is_balanced(self):
        ...
        def _is_balanced_aux(node):
            if node is None:
                balanced = True
            else:
                left_h = self._node_height(node._left)
                right_h = self._node_height(node._right)
                if abs(left_h - right_h) > 1:
                    balanced = False
                else:
                    balanced = _is_balanced_aux(node._left) and _is_balanced_aux(node._right)
            return balanced

        balanced = _is_balanced_aux(self._root)
        return balanced
    ...
    def retrieve_r(self, key):
        ...
        def _retrieve_r_aux(node, key):
            if node is None:
                result = None
            elif key < node._value:
                result = _retrieve_r_aux(node._left, key)
            elif key > node._value:
                result = _retrieve_r_aux(node._right, key)
            else:
                result = deepcopy(node._value)
            return result

        value = _retrieve_r_aux(self._root, key)
        return value
    ...
    def is_valid(self):
        ...
        def _is_valid_aux(node, min_val, max_val):
            if node is None:
                valid = True
            else:
                if (min_val is not None and node._value <= min_val) or \
                   (max_val is not None and node._value >= max_val):
                    valid = False
                else:
                    left_h = self._node_height(node._left)
                    right_h = self._node_height(node._right)
                    if node._height != max(left_h, right_h) + 1:
                        valid = False
                    else:
                        left_valid = _is_valid_aux(node._left, min_val, node._value)
                        right_valid = _is_valid_aux(node._right, node._value, max_val)
                        valid = left_valid and right_valid
            return valid

        valid = _is_valid_aux(self._root, None, None)
        return valid
    ...
    def inorder(self):
        ...
        def _inorder_aux(node, a):
            if node is not None:
                _inorder_aux(node._left, a)
                a.append(deepcopy(node._value))
                _inorder_aux(node._right, a)
            return

        a = []
        _inorder_aux(self._root, a)
        return a
    ...
    def preorder(self):
        ...
        def _preorder_aux(node, a):
            if node is not None:
                a.append(deepcopy(node._value))
                _preorder_aux(node._left, a)
                _preorder_aux(node._right, a)
            return

        a = []
        _preorder_aux(self._root, a)
        return a
    ...
    def postorder(self):
        ...
        def _postorder_aux(node, a):
            if node is not None:
                _postorder_aux(node._left, a)
                _postorder_aux(node._right, a)
                a.append(deepcopy(node._value))
            return

        a = []
        _postorder_aux(self._root, a)
        return a
